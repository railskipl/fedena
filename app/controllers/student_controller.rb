class StudentController < ApplicationController
  ...
  def academic_report
    ...
    @res      = HomeworkResult.find_all_by_homework_id(@homeworktypes)
    ...
    @course.examinations.each do |x|
     ...
     @course.homework.each do |x|
        @arr_total_wt[x.subject.name] += x.weightage
        ex_score = HomeworkResult.find_by_homework_id_and_student_id(x.id, @student.id)
        @arr_score_wt[x.subject.name] += ex_score.marks * x.weightage / x.max_marks unless ex_score.nil?
      end
  end
  ...
  def homework_report
     ...
     @graph = open_flash_chart_object(770, 350, "/student/graph_for_homework_report?course=#{@course.id}&homeworktype=#{@homeworktype.id}&student=#{@student.id}")
   end
  ...
  def update_student_result_for_homeworktype
      @student = Student.find(params[:student])
      ...
      @graph = open_flash_chart_object(770, 350, "/homework/graph_for_student_homework_result?course=#{@course.id}&homeworktype=#{@homeworktype.id}&student=#{@student.id}")
      render(:update) { |page| page.replace_html 'homework-results', :partial => 'student_result_for_homeworktype' }
    end
  ...
  def subject_wise_report
    ...
    @homeworktypes = @subject.homework_types
